...
- 分類はラジオボタンで選ぶ。Moodle の課題は科目ごとに分けず `Waseda` にまとめる

**Slack がタスクの置き場所**

- App Home に締切順のリスト。「開始」「完了」の 2 ボタンと、「…」に残りの操作
- 分類で絞り込める。選んだ状態はそのまま残る
- 上部に今日の学習時間・完了件数・連続日数。計測中は毎分描き直して経過時間が進む
- 完了は 24 時間以内なら「戻す」で取り消せる
- 締切の前日と 3 時間前にリマインド、毎朝ダイジェスト、毎週サマリ
- 同じタイミングの通知は必ず 1 通にまとめる

**Toggl で時間を測る**

- 「開始」で計測開始。分類がそのまま Toggl のプロジェクトになる
- 走っている計測があれば自動で止めてから始める
- ボタンを押すと先に画面が変わり、Toggl への反映はそのあとで追いかける
- 週次サマリに曜日別・分類別の学習時間、完了したタスク、来週の締切が出る
- 週次サマリは Toggl 側の実績も読む。Toggl のアプリから直接始めた計測も入る
...
**分類**（接続設定の下のほう）は、タスクを分ける単位で、そのまま Toggl のプロジェクト名になる。
1 行に 1 つ書く。**いちばん上が Moodle の課題の分類**で、既定は `Waseda`。
科目ごとにプロジェクトを分けると履修数だけ増えて週次サマリが読めなくなるため、大学の課題はここにまとめている。

**通知設定** からは、タイムゾーン・各通知の時刻・静音時間・週次サマリの曜日を変更できる。

Moodle を使わず、手動タスクと Toggl だけで使うこともできる。その場合は Moodle の欄を空のままにする。

設定を消したいときは、その欄に `-` だけを入れて保存する（空欄は「変更しない」の意味になるため）。
